...
CREATE TABLE user_facts (
  ...
  user_id UUID REFERENCES users(user_id) ON DELETE CASCADE,
  session_id TEXT NOT NULL REFERENCES sessions(session_id) ON DELETE CASCADE,
  fact_type TEXT NOT NULL, -- 'name', 'job', 'interest', 'preference', 'location', 'goal'
...
-- Usage tracking table (for cost control)
CREATE TABLE daily_usage (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(date, COALESCE(user_id::TEXT, session_id))
);
...
-- Create a function to clean up expired magic tokens (run this periodically)
...
-- RLS Policies (allow service role full access, anon key limited access)
-- Service role bypasses RLS, so these are mainly for logged-in users
...
-- Messages: can view messages from own sessions
...
-- Daily usage: can view own usage
...
-- Note: The backend will use the service role key for all operations
-- This bypasses RLS and allows full access, which is what we want
